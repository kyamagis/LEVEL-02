...
/*   Updated: 2022/08/29 12:43:48 by kyamagis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*dup_nl_to_null(char *saved_str)
{
	...
	if (saved_str[nl_point] == '\n')
		nl_point++;
	tmp_str = ft_strdup_gnl(&saved_str[nl_point]);
	ft_free_str(saved_str, CONTINUE);
	if (tmp_str == NULL)
		return (NULL);
	saved_str = tmp_str;
	return (saved_str);
...
static char	*dup_to_nl(char *saved_str)
{
	...
	if (output_line == NULL)
		return (NULL);
	ft_strlcpy_gnl(output_line, saved_str, nl_point + 1);
	return (output_line);
}
...
static char	*ft_join_to_null(char	*saved_str, char *buff)
{
	...
	if (joined == NULL)
		return (NULL);
	ft_strlcpy_gnl(joined, saved_str, len_sv + 1);
	ft_strlcpy_gnl(&joined[len_sv], buff, len_bf + 1);
	return (joined);
}
...
static char	*read_to_null(int fd, char *saved_str)
{
	...
	readsz = 1;
	while (0 < readsz && ft_strchr_gnl(saved_str, '\n') == NULL)
	{
		readsz = read(fd, buff, BUFFER_SIZE);
		if (readsz == -1)
			return (ft_free_str(saved_str, READ_ERROR));
		if (readsz == 0 && saved_str[0] == '\0')
			return (ft_free_str(saved_str, NOMAL_END));
		buff[readsz] = '\0';
		tmp_str = ft_join_to_null(saved_str, buff);
		if (tmp_str == NULL)
			return (ft_free_str(saved_str, MALLOC_ERROR));
		ft_free_str(saved_str, CONTINUE);
		saved_str = tmp_str;
	}
	return (saved_str);
...
char	*get_next_line(int fd)
{
	...
	if (saved_str == NULL)
		return (ft_free_str(output_line, MALLOC_ERROR));
	return (output_line);
}
